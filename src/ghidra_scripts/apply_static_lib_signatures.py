...
def ask_sig(sig_dir):
	try:
		...
		#sig_dir="/home/fra/Documents/sig-database-master/windows/"
		return [open(sig_dir+sig_file,'rb') for sig_file in listdir(sig_dir)]
...
sig_files=ask_sig(getScriptArgs()[0])

for i,sig_file in enumerate(sig_files):
	print('Parse Flirt File '+str(i+1)+'/'+str(len(sig_files)))
	try:
		flirt = parse_flirt_file(sig_file)
	except:
		print('Parsing Failed!')
	print('Name: ', flirt.header.library_name)
	print('Count:', flirt.header.n_functions)
	...
	apply_sig(flirt)
	print('[ %d / %d ]' % (rename_cnt, flirt.header.n_functions))
	print('Done '+str(i+1)+'/'+str(len(sig_files)))
